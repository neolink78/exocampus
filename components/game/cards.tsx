        ...
        <div className="flex justify-center mt-5">
            <div className={`grid ${size === '4x4' ? 'grid-cols-4' : 'grid-cols-4 sm:grid-cols-6'} gap-4 sm:gap-6 md:gap-10`}>
                {cards.map((card, index) => {
                    return (
                        <div
                            className={`relative flex justify-center items-center text-2xl font-bold text-black ${size === '4x4' ? 'w-14 h-14' : 'w-11 h-11 sm:w-20 sm:h-20'} lg:text-4xl lg:w-28 lg:h-28 transform cursor-pointer transition-transform duration-200 ${flipped.includes(index) || solved.includes(index)
